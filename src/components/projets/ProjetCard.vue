<template>
  <div class="row">
    <div
      class="col-xl-6 col-xxl-4 col-lg-6 col-md-6 col-sm-12"
      ...
      <div class="card custom-card border border-warning" data-aos="zoom-in">
        <div class="card-header p-2 d-block">
          <div class="row">
            <div class="col-xl-9 px-0 flex- text-center">
              <span class="fs-16 fw-bold text-uppercase">{{
                truncateText(projet.NomProjet, 55)
              }}</span>
            </div>

            <div class="col-xl-3 px-0 text-sm-center">
              <a
                :href="`https://project-files.org/bspp/public/api/fiche-projet/${projet.id}`"
                target="_blank"
              ...
              <a
                :href="`https://project-files.org/bspp/public/api/fiche-synoptique/${projet.id}`"
                target="_blank"
                class="btn btn-sm btn-icon btn-warning btn-wave ms-1"
                v-tippy="{
                  content: 'Voir la fiche synoptique du projet',
                  theme: 'custom',
                  animation: 'shift-away',
                }"
              >
              ...
              <router-link
                :to="{ name: 'detail-projet', params: { id: projet.id } }"
                class="btn btn-sm btn-icon btn-success btn-wave ms-1"
                v-tippy="{
        ...
        <div class="row py-2">
          <div class="col-3 text-center">
            ...
            <a
              href="#"
              class="mb-0 fw-semibold fs-20"
              style="color: #000"
              @click.prevent="showBailleurModal(projet)"
              data-bs-toggle="tooltip"
              :title="'Cliquez pour voir les détails des bailleurs'"
            >
          ...
          <div class="col-4 justify-content-center text-center">
            ...
            <a
              ...
              class="mb-0 fw-semibold fs-20"
              style="color: #000"
              @click.prevent="showRegionModal(projet)"
              data-bs-toggle="tooltip"
              :title="'Cliquez pour voir les détails des régions'"
            >
          ...
          <div class="col-5 justify-content-center text-center">
            ...
            <p class="text-dark mb-0">
              <span class="fs-20">{{
                formatBudget(projetBudget(projet))
              }}</span>
            </p>
          </div>
        </div>
          ...
          <div class="row bg-gray-200" style="width: 98%">
            <div class="col-6 text-center">
              ...
              <span
                class="fw-semibold"
                :style="{
                  color: getProgressColor(projetTauxDecaissement(projet)),
                }"
                >{{ Math.round(projetTauxDecaissement(projet)) }}%</span
              >
              <div
                class="progress mb-1"
                role="progressbar"
            ...
            <div class="col-6">
              <div class="mt-sm-0 mt-2 text-center">
                <span class="fs-16">Mise en œuvre (%):</span> <br />
                <span
                  class="fw-semibold"
                  :style="{
                  ...
                  >{{
                    Math.round(projet.suivis_dash?.TauxAvancementPhysique ?? 0)
                  }}%</span
                >
                <div
                  class="progress mb-1"
                ...
                >
                  <div
                    ...
                    :class="
                      getProgressClass(
                        projet.suivis_dash?.TauxAvancementPhysique
                      )
                    "
                    :style="{
                      width: projet.suivis_dash?.TauxAvancementPhysique + '%',
                    }"
                    :aria-valuenow="
                      projet.suivis_dash?.TauxAvancementPhysique ?? 0
                    "
              ...
              <div class="mt-sm-0 mt-2 text-center">
                ...
                >
                  <div
                    ...
                    :class="
                      getProgressClass(
                        projet.suivis_dash?.TauxExecutionFinanciere
                      )
                    "
                    :style="{
                      width: projet.suivis_dash?.TauxExecutionFinanciere + '%',
                    }"
                    :aria-valuenow="
                      projet.suivis_dash?.TauxExecutionFinanciere ?? 0
                    "
            ...
            <div class="col-4">
              <div class="mt-sm-0 mt-2 text-center">
                ...
                <span
                  class="fw-semibold"
                  :style="{
                    color: getProgressColor2(TauxConsommationDelai(projet)),
                  }"
                  >{{ Math.round(TauxConsommationDelai(projet)) }}%</span
                >
                <div
                  class="progress mb-1"
                  role="progressbar"
                ...
                >
                  <div
                    class="progress-bar progress-bar-striped"
                    :class="getProgressClass2(TauxConsommationDelai(projet))"
                    :style="{ width: TauxConsommationDelai(projet) + '%' }"
                    :aria-valuenow="TauxConsommationDelai(projet)"
                  ></div>
                </div>
              </div>
            </div>
  ...
  <bailleur-modal
    v-if="showModal"
    :bailleurs="currentBailleurs"
    @close="hideBailleurModal"
  />
  <region-modal
    v-if="showRegionModals"
    :regions="currentRegions"
    @close="hideRegionModal"
  />
</template>
<script>
